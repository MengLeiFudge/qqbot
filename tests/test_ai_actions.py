...
def test_ai_reply_review_codex_task_sends_only_short_private_result(tmp_path: Path) -> None:
    ...
    async def fake_runner(request):
        ...
        return CodexTaskResult(
            ...
            commit_subject="修复：收紧主动介入",
            commit_body="验证：pytest 已通过。",
        )
    ...
    assert "AI 回复自审已自动修复：qqbot" in message
    assert "提交：abcdef1 修复：收紧主动介入" in message
    assert "验证：pytest 已通过。" in message
    assert "结果记录：ARR-CODEX-" in message
    assert "完整日志：" in message
    assert "已安排 Bot 重启，重连后会回报状态。" in message

    log_path = tmp_path / "ai" / "reply_review_codex_results.jsonl"
    records = [json.loads(line) for line in log_path.read_text(encoding="utf-8").splitlines()]
    assert len(records) == 1
    assert records[0]["ok"] is True
    assert records[0]["project_id"] == "qqbot"
    assert records[0]["message"] == "Codex 完成。"
    assert records[0]["commit_hash"] == "abcdef1234567890"
    assert records[0]["commit_subject"] == "修复：收紧主动介入"
    assert records[0]["commit_body"] == "验证：pytest 已通过。"
    assert records[0]["prompt"] == "修一下机器人"


def test_ai_reply_review_codex_task_persists_full_failure_output(tmp_path: Path) -> None:
    bot = FakeBot()
    tasks = []
    ...
    async def fake_runner(request):
        ...
        return CodexTaskResult(
            False,
            "pytest 失败：test_ai_command.py::test_ai_named_trigger_requires_calling_bot",
            exit_code=1,
            stdout="stdout line\n" + "测试输出" * 400,
            stderr="stderr line\nAssertionError: named_trigger failed",
        )
    ...
    assert bot.calls[0][0] == "send_private_msg"
    message = str(bot.calls[0][1]["message"])
    assert "AI 回复自审自动修复失败：qqbot" in message
    assert "exit_code=1" in message
    assert "test_ai_command.py::test_ai_named_trigger_requires_calling_bot" in message
    assert "结果记录：ARR-CODEX-" in message
    assert "完整日志：" in message
    ...
    assert records[0]["stderr"] == "stderr line\nAssertionError: named_trigger failed"
    assert records[0]["stdout"].startswith("stdout line\n")
    assert records[0]["evidence"] == '{"has_issue": true}'

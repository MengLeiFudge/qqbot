...
- AI 接入：OpenAI-compatible 多 provider 配置、流式响应、群上下文、按群主动介入、复杂问题占位回复、领域知识候选、连续短回复、个人回复风格、RightCodes 生图、需求提案、Codex 会话中转
...
群聊普通 AI 对话默认仍需要 @ 机器人。Bot 管理员可以在后台面板的“AI 主动介入”区块按群开启或关闭；开启后，机器人只在保守判定为合适的时机主动参与。主动介入会按场景决定是否引用触发消息；需要知识库、联网、代码分析、图片生成、超过 800 字输入，或判定为不能一句话回答的问题，会先引用回复“我先看看”，再处理最终答案。复杂问题、数学/推理问题和强领域关联问题会优先保证准确性；如果等待期间群友已经给出一致答案，机器人会引用该答案并短确认，避免重复刷屏。

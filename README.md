...
Sub2API 用量查询归入 `astrbot_plugin_qqbot_features` 固定命令：群里发送 `用量` 会返回一张图片，包含全部 Sub2API 账号的 5h / 7d 额度，以及有消费用户的账号7d / 24h / 7d / 14d / 30d 实际扣费消费榜；账号7d位于第一项金额列。账号7d按每个 Sub2API 用户分别统计：当前窗口起点为原始账号 `seven_day.resets_at - 7 days`；起点当天从首个完整小时开始汇总小时趋势，后续日期按自然日汇总 `actual_cost`，多账号结果相加。账号和用户都由插件自动分页发现，账号改名、增加或删除后不需要改配置；用户显示名优先使用 username，为空时展示等长脱敏邮箱，四个自然日周期消费全部为 0 的用户不进入榜单。其余用户消费按 `Asia/Shanghai` 统计并使用 Sub2API `actual_cost`：24h 与 WebUI 一致，按昨日和今日两个日期查询；7d / 14d / 30d 分别是今天加前 6 / 13 / 29 个自然日。插件启动后按 `sub2api_refresh_interval_seconds` 后台定时刷新：用户榜和账号主动刷新各自每 300 秒一次、相隔半个周期，账号7d跟随账号定时刷新计算；群消息只读取最近一次缓存，不发起 Sub2API 请求。账号、账号7d或用户刷新失败时，图片会明确标出失败原因，并保留另一部分或上次成功缓存，不会把旧值伪装成新值。Sub2API 当前用户消费接口单次最多返回 200 人；超过时会明确报错，不会把未返回用户的消费误报为零。`sub2api_alert_group_ids` 可填写英文逗号分隔的 QQ 群号，任一账号 5h 用量首次跨过 80%、90%、95% 时自动提醒这些群；回落到阈值以下后才允许再次触发同一阈值。Sub2API 根地址和 Admin API Key 填在 AstrBot 插件运行态配置 `sub2api_base_url`、`sub2api_admin_api_key`，真实 key 不写入源码或示例配置。

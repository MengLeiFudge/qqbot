...
shapez 群 `1163635014` 会每天早上 8 点检测群文件最外层文件。文件夹内文件不处罚；最外层超过一周的旧文件会按上传者禁言 1 天，并私聊提示把文件清理掉或移动到合适文件夹。处理完成后私聊回复 `1`，机器人会重新读取群文件，复核通过后解除禁言；如果仍有旧文件留在最外层，会继续提示待处理文件。

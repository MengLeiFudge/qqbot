...
群聊普通 AI 对话默认仍需要 @ 机器人。Bot 管理员可以在后台面板的“AI 主动介入”区块按群开启或关闭；开启后，机器人只在保守判定为合适的时机主动参与。主动介入第一条回复会引用触发消息并 @ 提问人，后续连续短回复不重复引用。需要知识库、联网、代码分析、图片生成、超过 800 字输入，或判定为不能一句话回答的问题，会先引用回复“我先看看”，再处理最终答案。

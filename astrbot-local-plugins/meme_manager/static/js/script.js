document.addEventListener("DOMContentLoaded", () => {
  ...
  let latestEmojiData = {};
  let latestMemeIndex = { categories: {}, images: [] };
  ...
  const emojiEditModalTitle = document.getElementById("emoji-edit-modal-title");
  const emojiEditPreviewImg = document.getElementById("emoji-edit-preview-img");
  const emojiEditTitleInput = document.getElementById("emoji-edit-title-input");
  const emojiEditCaptionInput = document.getElementById("emoji-edit-caption-input");
  const emojiEditTagsInput = document.getElementById("emoji-edit-tags-input");
  const emojiEditUseInput = document.getElementById("emoji-edit-use-input");
  const emojiEditAvoidInput = document.getElementById("emoji-edit-avoid-input");
  const emojiEditIntensityInput = document.getElementById("emoji-edit-intensity-input");
  const emojiEditWeightInput = document.getElementById("emoji-edit-weight-input");
  const emojiEditAutoInput = document.getElementById("emoji-edit-auto-input");
  ...
  async function fetchEmojis() {
    ...
    try {
      const [emojiResponse, tagDescriptions, memeIndex] = await Promise.all([
        ...
        fetch("/api/index", {
          ...
          if (!res.ok) throw new Error("获取本地索引失败");
          return res.json();
        }),
      ]);
      ...
      latestMemeIndex = memeIndex || { categories: {}, images: [] };
      pruneSelectionState();
  ...
  function getEmojiMetadata(category, emoji) {
    const images = Array.isArray(latestMemeIndex.images)
      ? latestMemeIndex.images
      : [];
    return (
      images.find((entry) => entry.category === category && entry.filename === emoji) ||
      {
        category,
        filename: emoji,
        title: emoji.replace(/\.[^.]+$/, ""),
  ...
  function buildEmojiSearchText(category, emoji, metadata) {
    return [
      category,
      emoji,
      metadata.title,
      metadata.content_caption,
      ...(Array.isArray(metadata.emotion_tags) ? metadata.emotion_tags : []),
      ...(Array.isArray(metadata.use_cases) ? metadata.use_cases : []),
      ...(Array.isArray(metadata.avoid_when) ? metadata.avoid_when : []),
    ]
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
  ...
  function applyMemeSearchFilter() {
    const query = String(memeSearchInput?.value || "").trim().toLowerCase();
    const tokens = query.split(/\s+/).filter(Boolean);
    const categories = document.querySelectorAll(".category");

    categories.forEach((categoryDiv) => {
      const emojiItems = categoryDiv.querySelectorAll(".emoji-item");
      let visibleCount = 0;
      emojiItems.forEach((item) => {
        const haystack = item.dataset.search || "";
        const isVisible =
          tokens.length === 0 || tokens.every((token) => haystack.includes(token));
        item.classList.toggle("meme-search-hidden", !isVisible);
        if (isVisible) {
          visibleCount += 1;
        }
      });
      categoryDiv.classList.toggle(
        "meme-search-hidden",
        tokens.length > 0 && visibleCount === 0
      );
      const summary = categoryDiv.querySelector(".meme-search-summary");
      if (summary) {
        summary.textContent =
          tokens.length > 0 ? `筛选显示 ${visibleCount} 张` : "";
      }
    });

    if (memeSearchClearBtn) {
      memeSearchClearBtn.disabled = tokens.length === 0;
    }
  }
  ...
  function displayCategories(emojiData, tagDescriptions) {
    ...
    Object.entries(emojiData).forEach(([category, emojis]) => {
      ...
      searchSummary.className = "meme-search-summary";

      titleMain.appendChild(categoryName);
      titleMain.appendChild(selectionSummary);
      titleMain.appendChild(searchSummary);
      ...
      if (Array.isArray(emojis)) {
        emojis.forEach((emoji) => {
          const metadata = getEmojiMetadata(category, emoji);
          const emojiItem = document.createElement("div");
          emojiItem.className = "emoji-item";
          emojiItem.classList.toggle(
            "emoji-disabled",
            metadata.auto_send_enabled === false
          );
          emojiItem.dataset.category = category;
          emojiItem.dataset.emoji = emoji;
          emojiItem.dataset.search = buildEmojiSearchText(category, emoji, metadata);
          emojiItem.dataset.suppressClick = "false";
          emojiItem.tabIndex = 0;
          emojiItem.title = metadata.content_caption || metadata.title || emoji;
          ...
          metaOverlay.className = "emoji-meta-overlay";

          const metaTitle = document.createElement("span");
          metaTitle.className = "emoji-meta-title";
          metaTitle.textContent = metadata.title || emoji.replace(/\.[^.]+$/, "");
          metaOverlay.appendChild(metaTitle);

          if (metadata.auto_send_enabled === false) {
            const disabledBadge = document.createElement("span");
            disabledBadge.className = "emoji-meta-badge";
            disabledBadge.textContent = "禁发";
            metaOverlay.appendChild(disabledBadge);
          }

          const tags = Array.isArray(metadata.emotion_tags)
            ? metadata.emotion_tags.slice(0, 2)
            : [];
          if (tags.length > 0) {
            const tagLine = document.createElement("span");
            tagLine.className = "emoji-meta-tags";
            tagLine.textContent = tags.join(" / ");
            metaOverlay.appendChild(tagLine);
          }
          emojiItem.appendChild(metaOverlay);
  ...
  if (memeSearchClearBtn) {
    memeSearchClearBtn.addEventListener("click", () => {
      if (memeSearchInput) {
        memeSearchInput.value = "";
      }
      applyMemeSearchFilter();
  ...
  if (emojiEditCancelBtn) {
    emojiEditCancelBtn.addEventListener("click", () => {
      closeEmojiEditModal();
    });
  }
  ...
  if (emojiEditModalRoot) {
    emojiEditModalRoot.addEventListener("click", (event) => {
      if (event.target === emojiEditModalRoot) {
        closeEmojiEditModal();
      }
    });
  }
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Escape" && emojiEditModalRoot) {
      const isEmojiEditOpen = !emojiEditModalRoot.classList.contains("hidden");
      if (isEmojiEditOpen) {
        closeEmojiEditModal();
        return;
      }
  ...
  function closeEmojiEditModal() {
    ...
    activeEmojiEdit = null;
  }

  function editEmoji(category, emoji) {
    const metadata = getEmojiMetadata(category, emoji);
    activeEmojiEdit = { category, emoji };
    if (emojiEditModalTitle) {
      emojiEditModalTitle.textContent = `编辑「${metadata.title || emoji}」`;
    }
    if (emojiEditPreviewImg) {
      emojiEditPreviewImg.src = `/memes/${category}/${emoji}`;
      emojiEditPreviewImg.alt = metadata.title || emoji;
    }
    if (emojiEditTitleInput) {
      emojiEditTitleInput.value = metadata.title || emoji.replace(/\.[^.]+$/, "");
    }
    if (emojiEditCaptionInput) {
      emojiEditCaptionInput.value = metadata.content_caption || "";
    }
    if (emojiEditTagsInput) {
      emojiEditTagsInput.value = listToText(metadata.emotion_tags);
    }
    if (emojiEditUseInput) {
      emojiEditUseInput.value = listToText(metadata.use_cases);
    }
    if (emojiEditAvoidInput) {
      emojiEditAvoidInput.value = listToText(metadata.avoid_when);
    }
    if (emojiEditIntensityInput) {
      emojiEditIntensityInput.value = metadata.intensity || 2;
    }
    if (emojiEditWeightInput) {
      emojiEditWeightInput.value = metadata.weight || 1;
    }
    if (emojiEditAutoInput) {
      emojiEditAutoInput.checked = metadata.auto_send_enabled !== false;
    }
    if (emojiEditModalRoot) {
    ...
    window.setTimeout(() => {
      emojiEditTitleInput?.focus();
      emojiEditTitleInput?.select();
    }, 0);
  }

  async function saveEmojiMetadata() {
    if (!activeEmojiEdit) {
      showToast("未找到当前正在编辑的表情包。", "error", "保存失败");
      return;
    }
    const metadata = {
      title: emojiEditTitleInput?.value.trim() || "",
      content_caption: emojiEditCaptionInput?.value.trim() || "",
      emotion_tags: textToList(emojiEditTagsInput?.value || ""),
      use_cases: textToList(emojiEditUseInput?.value || ""),
      avoid_when: textToList(emojiEditAvoidInput?.value || ""),
      intensity: Number(emojiEditIntensityInput?.value || 2),
      weight: Number(emojiEditWeightInput?.value || 1),
      auto_send_enabled: Boolean(emojiEditAutoInput?.checked),
    };

    setButtonBusy(emojiEditSaveBtn, "保存中...");
    try {
      await requestJson(
        "/api/emoji/metadata",
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            category: activeEmojiEdit.category,
            filename: activeEmojiEdit.emoji,
            metadata,
          }),
        },
        { defaultErrorMessage: "保存单图信息失败" }
      );
      closeEmojiEditModal();
      await refreshUi({ emojis: true });
      showToast("单图信息已保存。", "success", "保存成功");
    } catch (error) {
      console.error("保存单图信息失败:", error);

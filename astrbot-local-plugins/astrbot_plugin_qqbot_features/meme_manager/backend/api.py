...
from ..local_index import (
    export_index_for_api,
    load_meme_index,
    migrate_mlj_pack_index,
...
async def get_meme_index():
    """获取本地表情索引。"""
    return jsonify(export_index_for_api()), 200
...
async def migrate_mlj_pack():
    ...
    try:
        ...
        return jsonify({"message": "迁移完成", **result}), 200
    except FileNotFoundError as exc:
        return jsonify({"message": str(exc)}), 404
    except Exception as exc:
        logger.exception("迁移 mlj_pack 表情索引失败: %s", exc)
        return jsonify({"message": f"迁移失败: {exc}"}), 500


@api.route("/emoji/metadata", methods=["POST"])
async def update_emoji_metadata():
    ...
    if not category or not filename or not isinstance(metadata, dict):
        return jsonify({"message": "category, filename and metadata are required"}), 400

    image_path = Path(MEMES_DIR) / category / filename

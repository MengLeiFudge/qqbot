...
async def call_split_text_api(
    ...
    **data: object,
    ...
    text = str(message)
    chunks = [text[index : index + 1000] for index in range(0, len(text), 1000)] or [""]
    for chunk in chunks:
        if api == "send_group_msg" and group_interval_sleep is not None:
            await group_interval_sleep(0)

...
MENU_SECTIONS: tuple[MenuSection, ...] = (
    MenuSection(
        ...
        lines=(
            ...
            "入群欢迎：云栖和夜凛各自按身份欢迎新成员；双 bot 互相入群不欢迎",
        ),
    ),
    MenuSection(
        name="棉花糖互动",
        ...
        status="混合功能",
        lines=(
            "AI对话：显式呼叫后按群激活 3 分钟，由主模型挑选后续消息并可主动静默",
            "RightCodes生图：棉花糖生图 提示词；生图模型；切换生图模型 模型名；查看积分 / 积分排行",
            "Sub2API用量：用量 按账号展示额度和各自当前7d周期用户榜，底部展示全账号当日 / 本周 / 30d 消费；后台定时刷新后直接读缓存",

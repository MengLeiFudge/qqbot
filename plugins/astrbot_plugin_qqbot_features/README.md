...
- `用量`
  - 发送一张图片，按账号依次展示 5h / 7d 额度卡和该账号自己的当前 7d 周期用户消费榜；全部账号区块结束后，再展示全账号近 24 小时 / 7 天 / 14 天 / 30 天滚动消费榜。
  - 每个账号榜的窗口起点为该账号原始 `seven_day.resets_at - 7 days`，按稳定 `account_id` 过滤：起点当天从首个完整小时开始汇总小时趋势，后续日期按自然日汇总 `actual_cost`；不同账号不再合计，每个榜显示该账号全部非零用户并按金额降序。底部全账号榜使用 Sub2API `actual_cost` 和 `Asia/Shanghai` 时区：24h / 7d / 14d / 30d 以同一次后台刷新时刻为终点，起点分别向前 1 / 7 / 14 / 30 天；起点当天同样从首个完整小时开始汇总小时趋势，后续日期按自然日汇总，起点侧误差不足一小时。
  - 账号和用户由插件自动分页发现；用户显示名优先使用 username，为空时展示等长脱敏邮箱；底部四个滚动周期消费全部为 0 的用户不进入全账号榜。

...
- `用量`
  - 发送一张图片，展示全部 Sub2API 账号的 5h / 7d 用量和有消费用户的 24h / 7d / 14d / 30d 实际消费榜。
  - 账号和用户由插件自动分页发现；用户显示名优先使用 username，为空时展示等长脱敏邮箱；四档消费全部为 0 的用户不进入榜单。用户消费使用 Sub2API `actual_cost` 和 `Asia/Shanghai` 日期口径：24h 与 WebUI 一致，按昨日和今日两个日期查询；7d / 14d / 30d 分别是今天加前 6 / 13 / 29 天。
  - 用户榜和 Sub2API `source=active&force=true` 账号刷新各自默认每 300 秒一次，错开半个周期；群里发送 `用量` 时只读取缓存，不等待刷新请求。账号或用户刷新失败时，图片会显示错误并保留上次成功数据。

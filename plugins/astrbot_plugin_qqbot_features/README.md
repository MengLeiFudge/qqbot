...
本插件是 AstrBot 当前固定功能入口，覆盖群务、菜单、生图、美图、表情管理、复读、养鲲、落樱之都、Arcaea、Factorio 和异形工厂。拍一拍不在这里生成特殊回复，只由 `astrbot_plugin_topic_concentration` 视为普通显式呼叫。
...
- `用量`
  - 发送一张图片，展示全部 Sub2API 账号的 5h / 7d 用量和有消费用户的账号7d / 24h / 7d / 14d / 30d 实际消费榜；账号7d位于第一项金额列。
  - 账号7d按每个 Sub2API 用户分别统计：当前窗口起点为原始账号 `seven_day.resets_at - 7 days`；起点当天从首个完整小时开始汇总小时趋势，后续日期按自然日汇总 `actual_cost`，多账号结果相加。其余用户消费使用 Sub2API `actual_cost` 和 `Asia/Shanghai` 日期口径：24h 与 WebUI 一致，按昨日和今日两个日期查询；7d / 14d / 30d 分别是今天加前 6 / 13 / 29 天。
  - 账号和用户由插件自动分页发现；用户显示名优先使用 username，为空时展示等长脱敏邮箱；四个自然日周期消费全部为 0 的用户不进入榜单。
  - 用户榜和 Sub2API `source=active&force=true` 账号刷新各自默认每 300 秒一次，错开半个周期；账号7d跟随账号定时刷新计算。群里发送 `用量` 时只读取缓存，不发起 Sub2API 请求。账号、账号7d或用户刷新失败时，图片会显示错误并保留上次成功数据。
...
- 闲聊、普通问答、显式呼叫和群聊激活窗口内的候选消息交给 AstrBot LLM 链路；仅 @ 当前 bot 和拍一拍当前 bot 都视为显式呼叫。仅 @ 没有正文时允许保留“怎么了？”这类完整呼叫应答，其他普通回复仍清理追问式收尾。只 @、引用或拍一拍其中一只时，目标自己处理，即使已有请求在运行也不由另一只代班。

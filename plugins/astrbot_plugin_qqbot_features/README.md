...
- `积分排行` / `积分排行榜`
  ...
  - 普通群消息累计积分时缓存最新群昵称；排行有昵称时只显示昵称，没有昵称时显示中间位星号脱敏的 QQ。
- `用量`
  - 发送一张图片，展示全部 Sub2API 账号的 5h / 7d 用量和全部用户的 7d / 30d 实际消费榜。
  - 账号和用户由插件自动分页发现；用户名为空时展示脱敏邮箱。用户消费按 `Asia/Shanghai` 自然日统计，7d 是今天加前 6 天，30d 是今天加前 29 天，使用 Sub2API `actual_cost`。

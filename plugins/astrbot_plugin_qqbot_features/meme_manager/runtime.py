...
class MemeManagerRuntime:
    ...
    async def start_webui(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if await self._check_port_active():
                ...
                if await self._check_port_active():
                    occupied_port = self.server_port
                    if await self._switch_to_available_webui_port(occupied_port):
                        logger.warning(
                            "端口 %s 被旧管理后台或其他进程占用，改用 %s 启动",
                            occupied_port,
                            self.server_port,
                        )
                    else:
                        raise RuntimeError(
                            f"端口 {occupied_port} 仍被占用，且未找到可用备用端口"
                        )
    ...
    async def _switch_to_available_webui_port(
        self, occupied_port: int, max_candidates: int = 20
    ) -> bool:
        """旧后台进程占住配置端口时，改用后续端口启动。"""
        for candidate in range(occupied_port + 1, occupied_port + max_candidates + 1):
            self.server_port = candidate
            if not await self._check_port_active():
                return True

        self.server_port = occupied_port
        return False
    ...
    def _build_meme_context_text(
        self, event: AstrMessageEvent, components: list | None = None
    ) -> str:
        """合并用户原消息和待发送文本，供本地 selector 做轻量匹配。"""
        parts = []
        try:
    ...
    async def _send_memes_streaming(self, event: AstrMessageEvent):
        ...
        try:
            ...
            for emotion in self.found_emotions:
                ...
                meme_path = select_meme_for_emotion(
                    emotion,
                    context_text=self._build_meme_context_text(event),
                )
                if meme_path is None:
                    continue
                meme_file = str(meme_path)
            ...
            if self.found_emotions:
                ...
                if random_value <= threshold:
                    ...
                    for emotion in self.found_emotions:
                        ...
                        meme_path = select_meme_for_emotion(
                            emotion,
                            context_text=self._build_meme_context_text(
                                event, cleaned_components
                            ),
                        )
                        if meme_path is None:
                            continue

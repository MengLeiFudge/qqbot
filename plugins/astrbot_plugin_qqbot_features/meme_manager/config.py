...
def get_plugin_data_dir(plugin_name: str | None = None) -> Path:
    ...
    if repo_data_path is not None:
        return (repo_data_path / "plugin_data" / resolved_plugin_name).resolve()
...
def _find_workspace_astrbot_data_path() -> Path | None:
    """非 AstrBot 进程中，从仓库结构推导 data/astrbot/data。"""
    for parent in PLUGIN_DIR.parents:
        candidate = parent / "data" / "astrbot" / "data"
        if candidate.is_dir():
            return candidate
    return None

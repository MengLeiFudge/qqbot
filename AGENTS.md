...
- shapez 群 `1163635014` 的群文件治理由 `group_file_cleanup_service.py` 负责：每天 Asia/Shanghai 08:00 后检测一次 OneBot `get_group_root_files` 返回的根目录 `files`，并通过 `folders` / `get_group_files_by_folder` 识别文件夹内文件；处罚只看根目录外层旧文件，文件夹内文件不处罚。最外层超过一周的旧文件按上传者禁言 3 天并私聊提示，待处理用户私聊回复 `1` 等确认词后复核，根目录旧文件已清理或移入文件夹才调用 `set_group_ban duration=0` 解禁。同一待处理周期只私聊一次，功能内跨用户私聊按 3 秒间隔发送；连续 3 天仍未处理时，直接删除残留外层文件，不删除文件夹内文件。该规则处于试运行阶段，疑问、建议或意见引导私聊群主萌泪酱。
...
- AI 回复在群聊中按场景决定是否引用并 @：ack、隔了较久、多人同时聊、回答图片/日志/报错或需要精确指向某个问题时应引用；但目标消息在最近 5 条群消息内时默认不引用，直接 @ 回复，较早消息才保留引用以避免指向不清。主动介入批处理允许回复最近一段群聊整体；只是轻量参与或总结时可以不引用。连续短回复通常只在第一条可能引用，后续不重复引用；如果上一轮 AI 回复等待期间已积压同群后续消息，处理这些后续消息时不要继续引用同一个旧触发消息。AI 文本被切成多条连续回复时，第一条立即发送，后续每条按该条字符数除以 6 秒等待后再发送。长文本走折叠消息。等待期间如果群友已给出一致且完整的答案，机器人应引用该答案短确认；如果答案不全或错误，只补充缺口或纠正错误点，不重复已说过的内容。私聊回复保持普通文本，避免把群聊交互形态带入私聊。

...
- AstrBot provider 选择、模型切换和回退链只归 AstrBot Core / 会话配置 / `provider_settings` 管理，本地插件不得再实现独立 provider 顺序、独立 fallback 或“判定专用模型”配置。`provider_ltm_settings.active_reply.enable` 默认关闭；本地插件不得再 patch `GroupChatContext.need_active_reply`，不得按定时、消息条数、摘要或普通群聊窗口做主动插话。`astrbot_plugin_topic_concentration` 只负责显式呼叫、同用户 follow-up 和双 bot 普通 LLM worker 调度：直接 @ 当前 bot、引用当前 bot或私聊直接进入当前 bot 普通 LLM；同时 @ 天使和恶魔的普通聊天，两只都必须各自用自己的身份回答，不得输出“我不能代替姐姐/妹妹回答”这类拒答；纯同时 @、`在吗`、`出来` 或 `说句话` 这类无实质正文场景，两只也各自短句应到，不转交、不追问用途。未明确指定身份但文本包含“棉花糖”时，先判断是否真正在叫机器人；“棉花糖很好吃”这类食物/物件说法必须跳过，“棉花糖，帮我生成一张图片”“棉花糖这个图片是哪个角色”这类请求应按群权重选择一只回复。模糊呼叫判定只允许调用 AstrBot 当前会话 provider，失败则不回复，不新增独立 provider 或 fallback。触发回复后，同群同用户进入 3 分钟 follow-up 窗口；后续普通消息不必继续 @，窗口绑定实际回复的 worker，最多 6 条普通后续消息，模型可用内部标记 `[[QQBOT_FOLLOWUP_END]]` 终止窗口，发送前必须清理该标记。

...
- AI 入口先生成 `MessageDecision`，区分是否回复、领域、难度、延迟策略和输出格式。需要知识库、联网、代码分析、图片生成、数学/严密推理、超过 800 字输入，或判定为不能一句话回答的问题，只有在群里明确 @ 机器人或私聊这类显式找机器人时才先引用原消息回复“我先看看”，并在 `run/ai/pending_tasks.json` 记录处理状态；普通主动介入和昵称点名不发送占位消息。速度优先，但复杂问题、数学题和强领域关联问题不能为了快牺牲准确性。

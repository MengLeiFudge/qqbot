...
- AstrBot 双平台 `both/full` 下，闲聊、普通问答和主动接话可以由两个 bot 按 worker 调度策略共同参与；固定命令默认只由 `QQBOT_ASTRBOT_COMMAND_OWNER` 指定账号执行，未设置时为恶魔账号 `2629227874`。固定命令必须按目标 @、owner 和 message_id/fallback claim 唯一执行；菜单、帮助、指令、生图、下载、群文件清理等固定命令不得因双平台重复执行。AI 或普通 LLM 判定“像固定功能”的自然语言请求只能提示真实指令和消耗/副作用，不能直接执行扣积分、写文件、上传、群管、下载或改存档。

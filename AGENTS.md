...
- 双平台共用本地表情包索引 `data\memes\mlj_pack\index.json`；轻松日常、玩梗、吐槽、撒娇和短情绪回复应优先使用表情，短情绪闲聊允许纯表情回复，`auto_send_enabled=false` 的敏感支付、涩涩慎用、待复核类别不得自动发送。AstrBot 侧通过 `scripts\sync-meme-pack.py` 同步到 `data\astrbot\data\plugin_data\meme_manager\` 和运行态配置，不改 Core、不删除旧图库目录。
- 本地 artifact 发布迁移到 `astrbot_plugin_local_artifact_api`，在 AstrBot `full` 模式下监听 `127.0.0.1:8080` 的兼容 `POST /admin/api/artifacts/publish-local`；它使用插件内发布服务和 `data\astrbot\data\plugin_data\qqbot_features_runtime` 发布状态，通过 AstrBot aiocqhttp OneBot 上传群文件，不改 AstrBot Core。AstrBot-only 启动会清理旧 8080 占用并接管该端口。

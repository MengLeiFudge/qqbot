...
- 普通激活候选的队列保护由 `astrbot_plugin_topic_concentration` 负责：每个 bot 同时只允许一个普通候选进入 AstrBot 会话队列；候选等待超过 30 秒、激活窗口过期或状态代际变化时必须静默丢弃。provider 调用前失效的候选不得调用主回复模型，调用期间失效的候选不得发送旧结果。显式 @、引用、命名呼叫、拍一拍和私聊不受该候选限制，仍按原规则排队处理。
...
- RightCodes 生图命令入口归入 `astrbot_plugin_qqbot_features`，不再新增独立固定命令插件；生图积分事实源使用 `data\astrbot\data\plugin_data\qqbot_features_runtime\db\qqbot_features.sqlite3`，旧 `ai\draw_points.json` 仅作为首次导入来源。积分事实源只保留每个 QQ 的当前积分和当前生图模型，不保留每日免费状态、历史累计消息数、`message_count` 或全局昵称字段；旧用户和无效模型回退到 `gpt-image-2`。插件内所有用户显示名必须统一由 `(group_id, QQ)` 的 SQLite 群昵称缓存解析：每条群消息更新群名片、QQ 昵称和时间；当前群依次使用群名片、QQ 昵称；当前群无记录时只使用其他群最新 QQ 昵称，不得使用其他群群名片；仍无记录时回退 QQ。积分排行只在该回退值上做前后三位保留的 QQ 脱敏；养鲲排行、Arc 猜歌等其他用户称呼不得直接读取事件昵称或拼接额外 QQ。不得读取、迁移或恢复历史群昵称 JSON、群上下文或积分记录昵称字段。AstrBot 负责普通群消息积分累计；双平台 `both/full` 下只有固定命令 owner 账号累计普通群消息积分，积分和模型选择跨群、跨天使/恶魔共用。`查看积分` 必须返回当前积分、当前模型、该模型单次消耗和模型查看/主切换指令提示，不展示隐藏别名。`积分排行` / `积分排行榜` 返回全局积分前 10。`切换生图模型 <模型名>` 是主切换指令，`生图模型 <模型名>` 是不进菜单的别名，中文关键字和模型名之间允许有空格或没有空格；模型切换必须走固定命令和双平台 command claim。`棉花糖生图 <提示词>` 只使用已保存模型，不得恢复命令内临时指定模型；生图从第一张开始正常扣分，不得恢复每日免费次数。RightCodes API Key 归 AstrBot 插件配置 `astrbot_plugin_qqbot_features.api_key`，直接在 WebUI/运行态插件配置填写；插件不得再读取 `QQBOT_AI_KEY_RIGHTCODES` 或旧 `.env` 作为生图 key 来源。明确生图命令中的上下文指代提示词可先用当前会话 AstrBot provider 整理为准确生图提示词，再扣积分调用 RightCodes；不得新增独立生图 rewrite provider、独立 fallback 或独立模型顺序。生图提示词整理只能读取当前命令、引用文本和当前/引用消息里的可用图片 URL/路径；只有 `[图片]` 占位、没有可用文字或图片时必须直接提示补充引用或完整提示词，不能让 LLM 猜图。生图成功、失败或超时失败必须引用原始生图请求；失败和超时失败必须退回已扣积分，并提示用户通过模型查看/切换指令重试，不得自动切换模型。
...
- AstrBot Core 为群聊请求生成以 `LLM 响应错误:` 开头的内部错误结果时，必须由 `astrbot_plugin_qqbot_features` 在 `on_decorating_result` 发送前清空，禁止把 provider、鉴权、连接、限流或超时错误发到群聊；当前出错 bot 使用自身 aiocqhttp 连接私聊固定主人 `605738729`，通知只包含当前 `self_id`、来源群号、归一化错误类型和安全摘要，不得包含群友原消息、prompt、上下文、凭据或上游原始错误正文。同一归一化问题按插件实例在两只 bot 和所有群之间共用 600 秒单调时钟冷却，期间最多通知一次；通知失败仍保持群结果为空。私聊请求的错误行为保持 AstrBot 原样；该守卫不得实现独立 provider retry/fallback，也不得修改 AstrBot Core 的 timeout 或重试设置；该守卫自己的通知与日志不得记录原始 provider 错误、key、token、secret、password、authorization、prompt 或用户正文。

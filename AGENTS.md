...
- 机器人风格由程序侧统一轮换，用户不能通过群聊或私聊设置、切换或查询备选人格；提示词只给当前人格，不暴露轮换机制、切换时间、其他人格或完整人格池。
- 备选人格只需要内部 `preset_id` 和完整展示名用于状态恢复，不维护面向用户的风格别名；不要为了“切换风格”指令增加 `侦探`、`少女侦探`、`橘雪莉风` 这类可见别名。
- 新增或替换备选人格前，必须做人工试聊验收，不能只依赖代码测试。每个人格至少进行 3-5 轮短对话，也就是 6-10 条消息，覆盖日常求助、轻量情绪、追问短结论、用户要求切换或询问其他人格等场景。
- 人工试聊通过标准：人格特征稳定且和现有人格不明显同质化；连续对话不漂移；口癖自然不过量，例如猫娘只在合适句尾带“喵”，不使用“喵呜”；不输出括号动作、舞台说明或过度表情符号；不暴露轮换、切换或备选人格信息；事实准确性、安全、隐私和权限边界不被角色扮演覆盖。
- 人工试聊不通过时，先调整对应人格提示词，再重新试聊同一人格；只有当前备选人格全部通过人工评估后，才能提交人格池变更。

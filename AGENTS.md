...
- AI 入口先生成 `MessageDecision`，区分是否回复、领域、难度、延迟策略和输出格式。需要知识库、联网、代码分析、图片生成、超过 800 字输入，或判定为不能一句话回答的问题，先引用原消息回复“我先看看”，并在 `run/ai/pending_tasks.json` 记录处理状态。
...
- AI 回复在群聊中优先引用并 @ 当前发言者；连续短回复只在第一条引用，后续不重复引用；长文本走折叠消息。私聊回复保持普通文本，避免把群聊交互形态带入私聊。

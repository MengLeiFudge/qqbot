...
- 群聊普通消息会先做旁路记录：`group_nick_cache.py` 负责记录群名片、最近群上下文、管理端消息日志和长期群聊记忆。AI 主动介入不再按群开关控制，所有群聊普通消息都先经宽松候选判定；普通非 @ 候选不逐条即时请求 AI，而是按群进入短暂缓冲，安静窗口后交给 AI 结构化判断是否接话。代码只保留安全、本地硬门槛、缓冲、冷却、自消息过滤、另一个 bot 普通消息过滤和明显第三方提及过滤；话题浓度由 AI 按“具体话题簇/聊天类型”判断，例如“图灵完备里面线路怎么接”“某种分馏塔怎么用”，不是求助词、诊断词或领域词的代码加权分。AI 判定失败时必须静默。敏感凭据风险提醒是例外，必须即时本地回复，不进普通 LLM 和主动缓冲。
...
- AI 表情包由 `features/ai/meme_selector.py` 在发送前根据 `data\memes\mlj_pack\index.json` 选择，最多追加 1 张，并受同群冷却限制。只允许 `auto_send_enabled=true` 的图片进入自动发送；敏感支付、涩涩慎用、待复核类别必须保留但禁用。技术、报错、安全、群管理、凭据提醒和长解释场景不自动附图，不让 LLM 直接生成 CQ 码。

...
"""兼容旧命令名：把 mlj_pack 迁移进 meme_manager 本地索引。
...
def main() -> int:
    if str(PLUGIN_SOURCE_ROOT.parent) not in sys.path:
        sys.path.insert(0, str(PLUGIN_SOURCE_ROOT.parent))

    from meme_manager.local_index import MEME_INDEX_PATH, MEMES_DIR, migrate_mlj_pack_index

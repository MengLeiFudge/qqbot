...
1. 运行时固定 jmcomic==2.7.2 和 img2pdf==0.6.3，版本清单位于 tools/runtime-scripts/astrbot-extra-requirements.txt。
...
5. 上游升级必须显式修改版本清单和适配器，运行离线契约测试后再做一个真实作品下载、PDF 生成和 OneBot 上传探针；探针失败时不发布该升级。
6. 漫画任务只使用 AstrBot Core temp 下的独立 job 目录。上传完成、失败或超时均清理，不建立持久内容缓存。
...
- PDF 编码增加一个固定依赖，但比 Pillow 一次性持有大量页面更适合大作品，并由自有接口保持可替换。

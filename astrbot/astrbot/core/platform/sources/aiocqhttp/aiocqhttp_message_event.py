...
class AiocqhttpMessageEvent(AstrMessageEvent):
    ...
    def _normalize_plain_text(text: str) -> str:
        # Keep short social replies even when upstream prompts drift into lists.
        normalized = text.replace("\r\n", "\n").replace("\r", "\n")
        normalized = re.sub(r"\n[ \t\n]*\n", "\n", normalized)

        lines = [line.strip() for line in normalized.split("\n") if line.strip()]
        selected: list[str] = []
        skip_short_list = False
        for line in lines:
            if line.endswith((":", "：")):
                skip_short_list = True
                continue
            ...
            if len(selected) >= 2 or len(line) >= 28:
                break

        if not selected and lines:
            selected.append(lines[0])
        normalized = "\n".join(selected)
        if len(normalized) > AiocqhttpMessageEvent._MAX_PLAIN_TEXT_CHARS:
            sentence_match = re.match(r"^(.{1,40}?[。！？!?])", normalized)
            if sentence_match:
                return sentence_match.group(1).strip()
            normalized = (
                normalized[: AiocqhttpMessageEvent._MAX_PLAIN_TEXT_CHARS - 1].rstrip()
                + "…"
            )
        return normalized
        ...
        for segment in message_chain.chain:
            if isinstance(segment, At):
                ...
                if not segment.text.strip():
                    continue
                d = await AiocqhttpMessageEvent._from_segment_to_dict(segment)
                d["data"]["text"] = AiocqhttpMessageEvent._normalize_plain_text(
                    d["data"]["text"],
                )
                if not d["data"]["text"].strip():
                    continue
                ret.append(d)
            else:
                d = await AiocqhttpMessageEvent._from_segment_to_dict(segment)
